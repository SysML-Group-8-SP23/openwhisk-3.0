...
class ShardingContainerPoolBalancer(
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val finalMsg = ActivationMessage(msg.transid, msg.action, msg.revision, msg.user, msg.activationId, msg.rootControllerIndex, msg.blocking, updatedMsgContent, msg.initArgs, msg.lockedArgs, msg.cause, msg.traceContext)
    ...
    chosen
      .map { invoker =>
        ...
        logging.info(
          this,
          s"scheduled activation ${finalMsg.activationId}, action '${finalMsg.action.asString}' ($actionType), ns '${finalMsg.user.namespace.name.asString}', mem limit ${memoryLimit.megabytes} MB (${memoryLimitInfo}), time limit ${timeLimit.duration.toMillis} ms (${timeLimitInfo}) to ${invoker}")
        val activationResult = setupActivation(finalMsg, action, invoker, action.limits.cpu.cores)
