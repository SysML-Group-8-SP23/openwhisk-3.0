 ...
 */
class ServerlessProjectScheduler(
  config: WhiskConfig,
  ...
  val schedulingState = ServerlessProjectSchedulerState()(lbConfig)

  /**
   * Monitors invoker supervision and the cluster to update the state sequentially
   *
   * All state updates should go through this actor to guarantee that
   * [[ServerlessProjectSchedulerState.updateInvokers]] and [[ServerlessProjectSchedulerState.updateCluster]]
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val hash = ServerlessProjectScheduler.generateHash(msg.user.namespace.name, action.fullyQualifiedName(false))
...
object ServerlessProjectScheduler extends LoadBalancerProvider {
  ...
  override def instance(whiskConfig: WhiskConfig, instance: ControllerInstanceId)(implicit actorSystem: ActorSystem,
    ...
    new ServerlessProjectScheduler(
      whiskConfig,
      instance,
        ...
        if (dispatchedMem(invoker.id.toInt).tryAcquireConcurrent(fqn, maxConcurrent, slots)) {
          if (dispatchedCpu(invoker.id.toInt).tryAcquireConcurrent(fqn, maxConcurrent, cores)) {
            ...
            // Couldn't get requested cores, so release memory slots obtained
              ...
              dispatchedCpu(invoker.id.toInt).releaseConcurrent(fqn, maxConcurrent, cores)
            }
          // Couldn't get requested cores, so release memory slots obtained
          } else {
            dispatchedMem(invoker.id.toInt).releaseConcurrent(fqn, maxConcurrent, slots)
   ...
   * @param dispatchedBandwidth semaphores for each invoker to give the bandwidth away from
  ...
  def schedule(
    ...
    if (numInvokers > 0) {
      if (stepsDone == numInvokers || finalRule == 1) {
        ...
        var bestDiffBandwidth = chosenDifferenceBandwidth
        ...
        if (!invokerMap.isEmpty) {
          logging.warn(this, s"Lachesis - invokermap: '${invokerMap}'")
          // Search for best container on the Invoker
          for ((mapFqn, count) <- invokerMap) {
            logging.warn(this, s"Lachesis - container: '${mapFqn}' and '${count}'")
            if (currentRule !=1) {
              ...
              if ((mapFqnName == currFqnName) && (Math.abs(mapFqnMbps - bandwidth) <= 200)) {
                ...
                // Case #2: Found a warm container that is closest in memory and has enough cores compared to the requested size
                  ...
                  if (getResources(invoker, dispatchedMem, dispatchedCpu, dispatchedBandwidth, mapFqn, maxConcurrent, mapFqnMem, cores, bandwidth)) {
                    ...
                    bestDiffCpu = diffCpu
                    bestDiffMem = diffMem
                    bestDiffBandwidth = diffBandwidth
                    bestFqn = mapFqn
                    currentRule = 2
                  }
                }
 ...
 * @param _invokerBandwidth state of accessible bandwidth of each invoker
 */
case class ServerlessProjectSchedulerState(
  ...
  private var _managedStepSizes: Seq[Int] = ServerlessProjectScheduler.pairwiseCoprimeNumbersUntil(0),
  private var _blackboxStepSizes: Seq[Int] = ServerlessProjectScheduler.pairwiseCoprimeNumbersUntil(0),
  ...
  protected[loadBalancer] var _invokerCores: IndexedSeq[NestedSemaphore[FullyQualifiedEntityName]] =
    IndexedSeq.empty[NestedSemaphore[FullyQualifiedEntityName]],
  protected[loadBalancer] var _invokerBandwidth: IndexedSeq[NestedSemaphore[FullyQualifiedEntityName]] =
    IndexedSeq.empty[NestedSemaphore[FullyQualifiedEntityName]],
  ...
  def invokerBandwidth: IndexedSeq[NestedSemaphore[FullyQualifiedEntityName]] = _invokerBandwidth
  ...
  def updateInvokers(newInvokers: IndexedSeq[InvokerHealth]): Unit = {
    ...
    val logDetail = if (oldSize != newSize) {
      _managedStepSizes = ServerlessProjectScheduler.pairwiseCoprimeNumbersUntil(managed)
      _blackboxStepSizes = ServerlessProjectScheduler.pairwiseCoprimeNumbersUntil(blackboxes)
      ...
      if (oldSize < newSize) {
        ...
        _invokerCores = _invokerCores ++ onlyNewInvokers.map { invoker =>
          new NestedSemaphore[FullyQualifiedEntityName](160)
        }
        _invokerBandwidth = _invokerBandwidth ++ onlyNewInvokers.map { invoker =>
          new NestedSemaphore[FullyQualifiedEntityName](2000)
        }
  ...
  def updateCluster(newSize: Int): Unit = {
    ...
    if (_clusterSize != actualSize) {
      ...
      _invokerBandwidth = _invokers.map { invoker =>
        new NestedSemaphore[FullyQualifiedEntityName](2000)
      }
      ...
      val totalInvokerCore = 
        _invokers.foldLeft(0L)((total, invoker) => total + 160)
      ...
      val totalInvokerBandwidth = 
        _invokers.foldLeft(0L)((total, invoker) => total + 2000)
      val averageInvokerBandwidth = 
        if (totalInvokerBandwidth > 0 && invokerCount > 0) {
          (totalInvokerBandwidth / invokerCount)
        } else {
          0
        }
      logging.info(
...
//   if (1rces(invoker, dispatchedMem, dispatchedCpu, mapFqn, maxConcurrent, slots, cores)) {
