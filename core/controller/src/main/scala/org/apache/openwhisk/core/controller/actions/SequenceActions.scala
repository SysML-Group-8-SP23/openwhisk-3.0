  ...
  private def makeSequenceActivation(user: Identity,
    ...
    val sequenceLimits = accounting.maxMemory map { maxMemoryAcrossActionsInSequence =>
      Parameters(
        ...
        ActionLimits(action.limits.timeout, MemoryLimit(maxMemoryAcrossActionsInSequence MB), action.limits.cpu, action.limits.bandwidth, action.limits.logs).toJson)
    }
