...
protected[actions] trait PrimitiveActions {
  ...
  private def completeActivation(user: Identity,
    ...
    val sequenceLimits = Parameters(
      ...
      ActionLimits(session.action.limits.timeout, MemoryLimit(session.maxMemory), session.action.limits.cpu, session.action.limits.bandwidth, session.action.limits.logs).toJson)
